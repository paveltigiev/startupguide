...
export default {
  ...
  methods: {
    addToFriends(){
      this.$store.dispatch('addToFriends', this.id)
    }
  },
  computed: {
    user() {
  ...
  watch: {
    user() {
      let arr = []
      this.user.payload.skills.forEach(item => {
        arr.push(item.name)
      })
      this.skills = arr.join(', ')
    }
  },
  created() {
